...
SequenceObject = "AdditiveSequence | tuple[AdditiveSequence, ...]"
...
class _ParentTable(Generic[Element]):
    ...
    __slots__ = ("elements", "index", "generators", "generator_indices")
    ...
    def __init__(self, parent) -> None:
        ...
        try:
            ...
            generators = ()
        if not set(generators) <= set(self.index):
            generators = ()
        self.generators = generators
        self.generator_indices = tuple(self.index[g] for g in generators)
...
def _identity_table(size: int) -> tuple[int, ...]:
...
class AutomorphismGroup(Generic[Element, Auto]):
    ...
    def __init__(
        ...
        *,
        ...
        if identity is None:
            identity = next((e for e in ordered if self._is_identity_element(e)), None)
            if identity is None:
        ...
        if generators is None:
            ...
            if compose is not None and not self._generates(self._generators):
                raise ValueError("the supplied generators do not generate the group")

    def _generates(self, generators: tuple[Auto, ...]) -> bool:
        seen = {self._identity}
        ...
        return len(seen) == len(self._element_set)

    # structure

    def _is_identity_element(self, element: Auto) -> bool:
        probe = getattr(element, "is_identity", None)
        if callable(probe):
            return bool(probe())
        return False
    ...
    def _traverse(self, obj, *, ordered: bool) -> dict:
        """Breadth-first orbit of ``obj`` under the generators: image -> word."""
    ...
    def _materialize(self, word: tuple[int, ...]) -> Auto:
    ...
    def orbit(self, obj, *, ordered: bool = False) -> tuple:
        ...
        _check_object(obj)
        return tuple(sorted(self._traverse(obj, ordered=ordered)))
    ...
    def transporter(self, source, target, *, ordered: bool = False) -> Auto | None:
        ...
        _check_object(source)
        if isinstance(target, tuple) and not ordered:
    ...
    def canonical(self, obj, *, ordered: bool = False):
        ...
        _check_object(obj)
        return min(self._traverse(obj, ordered=ordered))
    ...
    def canonical_form(
        ...
        _check_object(obj)
        words = self._traverse(obj, ordered=ordered)
        image = min(words)
        if witness == "smallest":
            element = self.transporter(obj, image, ordered=ordered)
        elif witness == "word":
            element = self._materialize(words[image])
        else:
            raise ValueError("witness must be 'smallest' or 'word'")
        permutation = None
        if isinstance(obj, tuple) and not ordered:
            raw = self.apply(element, obj, ordered=True)
            positions: dict = {}
            for position, item in enumerate(image):
                positions.setdefault(item, deque()).append(position)
            permutation = tuple(positions[item].popleft() for item in raw)
    ...
    def orbit_representatives(self, objects: Iterable, *, ordered: bool = False) -> dict:
        ...
        for obj in objects:
            _check_object(obj)
            key = tuple(sorted(obj)) if isinstance(obj, tuple) and not ordered else obj
            representative = canonical_of.get(key)
            if representative is None:
                orbit = self._traverse(key, ordered=ordered)
    ...
    def closure(
        ...
        identity = _identity_table(len(data.elements))
        seen = {identity}
...
def _check_object(obj) -> None:
    if isinstance(obj, AdditiveSequence):
        return
    if isinstance(obj, tuple) and all(isinstance(item, AdditiveSequence) for item in obj):
        if obj and any(item.parent() is not obj[0].parent() for item in obj):
    ...
    if group is None:
        ...
        if action is not None and callable(is_finite) and is_finite():
            group = AutomorphismGroup.closure(
                parent,
                [
                    (lambda index: lambda term: action.apply_term(index, term))(i)
